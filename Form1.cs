...
namespace Laboratory_work2
{
	public partial class Form1 : Form
	{
        ...
        int I;											// êîëè÷åñòâî øàãîâ ïî õ è âðåìåíè
		double L = 7, T = 1, tau = 0.01, h = 0.2;		// äëèíà ñòåðæíÿ, âðåìÿ âîçäåéñòâèÿ, øàã ïî âðåìåíè, øàã âäîëü ñòåðæíÿ (ñòàíäàðòíûå çíà÷åíèÿ)
		double[] fi = { 0, 0, 0 }, 
			bx = { -1.0339, -0.0036, -0.0036 };			// êîýôôèöèåíòû ôóíêöèé
		PointF[] funY, funY_0;
		...
		private double[] diffWarm(double[] Y)
		{
			...
			double[,] A = new double[I, I];     // ìàòðèöà êîýôôèöèåíòîâ
			double[] y;// = new double[I];         // çíà÷åíèÿ ôóíêöèè äëÿ ïðåäûäóùåãî ñëîÿ
			double[] f = new double[I];         // çíà÷åíèÿ ïðàâîé ÷àñòè óðàâíåíèÿ
			...
			while(true)
			{
				// ðàñ÷åò ïðàâîé ÷àñòè äëÿ íà÷àëüíûõ óñëîâèé
				f[0] = f[I - 1] = 0;
				for (int i = 1; i < I - 1; i++)
				{
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			//if ((tau / (h * h)) >= 0.25)
			//{
			//	MessageBox.Show("Íàðóøåíèå óñòîé÷èâîñòè!\n" +
			//	"Èçìåíèòå çíà÷åíèå h íà áîëüøå ÷åì: " + 2 * Math.Pow(tau, 0.5) +
			//		"\nèëè tau íà ìåíüøå ÷åì: " + 0.25 * h * h);
			//         }
			//         else
			//         {
